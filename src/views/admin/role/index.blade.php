...
@section('content')
	...
	<ol class="breadcrumb">
		...
		<li class="active">Alle rollen</li>
	</ol>
	<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
	<!--End breadcrumb-->
...
@section('scripts.footer')
	...
	<script>
		var tableConfig = {
			...
			urlData: '{{route('admin::role.all.data')}}',

			urlSort: '{{ route('admin::sitemap.sort')}}',
			urlWiDeleteRole: '{{ route('admin::role.destroy')}}',
			...
			columns: [
				...
				{data: 'id', name: 'roles.id',visible:false},

				{data: 'name', name: 'roles.name',searchable: true,xwidth:'1%' },
				{data: 'label', name: 'roles.label',searchable: true,xwidth:'1%' },
				{data: 'description', name: 'roles.description',searchable: true,xwidth:'40%' },

				{data: 'updated_at', name: 'roles.updated_at',searchable: true,width:'1px' },
				{data: 'action', name: 'action', orderable: false, searchable: false,visible:true,width:'1px'}
			],
			"language": {
				"lengthMenu": "Toon _MENU_  rollen per pagina",
				"zeroRecords": "Geen rol gevonden",
				//"info": "Toon pagina _PAGE_ van _PAGES_ _TOTAL_",
				"info" :  "Toon _START_ t/m _END_ van _TOTAL_ items",
				"infoEmpty": "Geen rollen beschikbaar",
